...
public class Transformer
{
    ...
    {
        App.TryEnqueueOnUI(() => this.AdjustWindowPosition(window, null));
    }
    ...
    public void AdjustScale(Window window, bool adjustPosition)
    {
        ...
        if (packet != null)
        {
            App.TryEnqueueOnUI(() => this.AdjustScale(packet, adjustPosition));
        }
    }
    ...
    { // UI thread
        var element = ((FrameworkElement)packet.Window.Content).FirstLogicalFrameworkElement();
        if (element == null)
        {
            return;
        }
    ...
    private void InitialProcess(Window window, TransformerObject obj)
    {
        if (obj.InitialPacket != null)
        {
            ...
            obj.InitialDesiredWidth = window.Content.DesiredSize.Width;
            obj.InitialDesiredHeight = window.Content.DesiredSize.Height;
    ...
    private void ProcessPacket(TransformerPacket? packet)
    {
        ...
        App.TryEnqueueOnUI(() => this.ProcessPacket_Do(packet));
    }
